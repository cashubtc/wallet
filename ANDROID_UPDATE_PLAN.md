...
Focused validation used during Android native wallet local-mint instrumentation:
...
# Attach a normal emulator/device, then forward device-local ports to the host mints:
adb -s <attached-emulator-serial> reverse tcp:3338 tcp:3338
adb -s <attached-emulator-serial> reverse tcp:3339 tcp:3339

# From the repository root, generate a fresh standalone invoice for the melt leg:
MELT_INVOICE="$(CI/.nutshell-venv/bin/python CI/generate-bolt11-invoice.py --amount-sats 3)"

# From android/:
ANDROID_SERIAL=<attached-emulator-serial> JAVA_HOME="$JAVA_HOME" ./gradlew --no-daemon :app:connectedDebugAndroidTest \
  -Pandroid.testInstrumentationRunnerArguments.class=org.cashu.wallet.liveintegration.NativeWalletLocalMintInstrumentedTest \
  -Pandroid.testInstrumentationRunnerArguments.cashu.nativeWalletLocalMintIntegration=true \
  -Pandroid.testInstrumentationRunnerArguments.cashu.nutshellMintUrl=http://127.0.0.1:3338 \
  -Pandroid.testInstrumentationRunnerArguments.cashu.cdkMintUrl=http://127.0.0.1:3339 \
  -Pandroid.testInstrumentationRunnerArguments.cashu.bolt11MeltInvoice="$MELT_INVOICE"
```

Notes:

- Native local-mint instrumentation uses the Android CDK runtime and therefore covers wallet operations that host JVM integration cannot load from `libcdk_ffi`.
- `adb reverse` plus `127.0.0.1` is the preferred connected-test path for local mints; `10.0.2.2` can be used only when the emulator host bridge is known to work.
- The debug build has a debug-only network security config for `localhost`, `127.0.0.1`, and `10.0.2.2`; the release manifest still forbids cleartext opt-in.
- Generate a fresh BOLT11 invoice for each full melt run. Reusing an already-paid invoice can correctly fail as a duplicate/paid invoice.
...
Connected-emulator instrumentation validation used during the UI bug sweep:

```sh
JAVA_HOME="$JAVA_HOME" ./gradlew --no-daemon :app:compileDebugKotlin :app:compileDebugAndroidTestKotlin :app:packageDebug :app:packageDebugAndroidTest
adb install -r app/build/outputs/apk/debug/app-debug.apk
adb install -r app/build/outputs/apk/androidTest/debug/app-debug-androidTest.apk
adb shell am instrument -w -r com.jcashu.wallet.debug.test/androidx.test.runner.AndroidJUnitRunner
```
...
Managed-device recovery used after emulator 36.6.11 rejected AGP's default `auto-no-window` GPU mode:

```sh
# Persisted in android/gradle.properties:
android.testoptions.manageddevices.emulator.gpu=swiftshader_indirect

# If a stale managed snapshot leaves ADB offline:
JAVA_HOME="$JAVA_HOME" ./gradlew --no-daemon :app:cleanManagedDevices

# Optional local timeout extension while rebuilding the managed snapshot:
JAVA_HOME="$JAVA_HOME" ./gradlew --no-daemon :app:pixel2Api35DebugAndroidTest \
  -Pandroid.experimental.testOptions.managedDevices.setupTimeoutMinutes=15
```

Current local status: direct connected-emulator instrumentation passes through AndroidJUnitRunner with 49/49 tests green after fixing the fake Compose shell, compact-height scrolling probes, settings toggle state, and mint swipe idempotence. Standard Gradle `:app:connectedDebugAndroidTest` also collects results successfully when run against a normal attached AVD selected with `ANDROID_SERIAL=<attached-emulator-serial>`; the full suite finished 51 tests with 1 opt-in native matrix test skipped and 0 failures, and the focused native wallet local-mint matrix passed when explicitly enabled against forwarded Nutshell/CDK mints. Gradle-managed `:app:pixel2Api35DebugAndroidTest` now also passes after setting `android.testoptions.manageddevices.emulator.gpu=swiftshader_indirect`; the managed suite finished 51 tests with 1 opt-in native matrix test skipped and 0 failures. SwiftPM iOS integration tests in `CI/IntegrationTests` pass 12/12 against the hermetic local Nutshell FakeWallet mint. Xcode project discovery now resolves Swift packages and lists the `CashuWallet`, `CashuWalletTests`, and `CashuWalletUITests` targets plus the `CashuWallet` scheme.
...
Milestone update: Android now has committed AGP-consumed baseline and startup profile sources under `android/app/src/main/baselineProfiles/`. The baseline profile seeds startup, Settings, tab switching, Send/Receive, scanner, and Home/History/Mints/Settings list surfaces, while a focused JVM guard keeps those journey descriptors present. Android also has a debug-only JankStats hook in `MainActivity`, debug-only recomposition probes in Home and Settings, and a self-instrumenting `:macrobenchmark` module with startup, Settings open/scroll/toggle, and Home/History/Mints list-scroll frame-timing journeys. A local API 35 emulator/debug smoke run passed 3/3 with `compilationMode=none` and explicit emulator/debug suppressions: cold startup median 414.8 ms, Settings frame CPU P99 10.8 ms with frame overrun P99 -1.0 ms, and Home/History/Mints frame CPU P99 11.1 ms with frame overrun P99 -1.3 ms. Release-device benchmarking should still be repeated on physical hardware before public release.
...
- `cd android && JAVA_HOME="$JAVA_HOME" ./gradlew --no-daemon :app:assembleDebug :macrobenchmark:assembleDebug`, then `adb shell am instrument -w -e targetPackage com.jcashu.wallet.debug -e compilationMode none -e androidx.benchmark.suppressErrors EMULATOR,DEBUGGABLE org.cashu.wallet.benchmark/androidx.test.runner.AndroidJUnitRunner`

Checklist:

- [x] Profile Settings open/scroll/toggle paths with Macrobenchmark, JankStats, and Compose recomposition counts before declaring performance complete. `android/macrobenchmark` is now self-instrumenting, accepts `targetPackage` and `compilationMode` instrumentation args, the target app includes ProfileInstaller for release Partial compilation, `android/PERFORMANCE_PROFILING.md` documents Gradle and local smoke commands, `MainActivity` logs debug-only JankStats, and `DebugRecompositionProbe` logs Home/Settings recomposition counts. Local debug smoke measured Settings frame CPU P99 10.8 ms and frame overrun P99 -1.0 ms on the API 35 emulator.
...
- [x] Profile Home list masking/fade drawing and animated QR generation; move QR bitmap generation off the main thread or add frame caching if animated UR/QR display causes missed frames. `WalletMacrobenchmark.homeHistoryAndMintsListScrollFrameTiming` covers Home/History/Mints list frame timing and passed local debug smoke with frame CPU P99 11.1 ms and frame overrun P99 -1.3 ms. Responsive QR sizing and existing background QR bitmap generation did not show frame overruns in the covered list/QR-adjacent flows; release-device QR/animated-UR spot checks remain part of the manual walkthrough.
...
- [x] Add large-font screenshot tests for Home, Unified Send, Send Ecash, Receive Ecash, Receive Lightning, Settings, Nostr, P2PK, Lightning, Mints, Mint Detail, and Transaction Detail. `FakeWalletVisualRegressionComposeTest` now captures each app-level fake screen in its own large-font probe so the Compose rule never resets content mid-test.
- [x] Add compact-height screenshot tests for amount entry/keypad screens and NFC/scanner overlays. `FakeWalletVisualRegressionComposeTest` captures compact Send, Receive, scanner, and contactless overlay probes, with scroll-to-action assertions for constrained heights.
- [x] Add tests that verify primary CTAs remain reachable above keyboard and navigation bars. `ButtonsComposeTest` and `FakeWalletVisualRegressionComposeTest` assert compact large-font CTAs remain usable for primary send/receive and overlay close actions.
...
- [x] Complete the Android UI Bug Audit Backlog above before declaring visual polish complete. Back gesture, predictive-back compatibility, clipping/insets/scrolling, Settings performance, interaction-state, and UI regression coverage are now implemented or covered by direct instrumentation, Macrobenchmark/JankStats/recomposition probes, and focused JVM guards.
...
- Broad fake app-level Compose suites now cover shell navigation, core screen stories, large-font visual probes, compact overlays, dark theme, and wide-width smoke images; managed-device execution passes after the managed emulator GPU override.
- Android now has CI/Nutshell/CDK-style endpoint/parser integration through `:app:androidLocalMintIntegrationTest` and native wallet-operation instrumentation through `NativeWalletLocalMintInstrumentedTest`.
- App-level instrumentation now covers onboarding create/restore, scanner/contactless overlays, backup, receive/send happy paths, mints, history, settings, accessibility, large font, compact height, dark theme, and wide width without real services.
- Screenshot/accessibility regression probes now exist in `FakeWalletVisualRegressionComposeTest` and `AccessibilitySemanticsComposeTest`; direct connected-emulator execution, standard Gradle connected result collection, and Gradle-managed-device result collection pass.

Unit test checklist:

Milestone update: JVM coverage now includes payment request/locked receive encoding, dedicated `CashuRequestStore` persistence tests for quote-intent upsert, attach by quote id, duplicate suppression, update, delete, reset, reload, stale current-id cleanup, legacy payment normalization, WalletManager startup maintenance/keyset refresh orchestration, Settings runtime-toggle foreground maintenance behavior, CDK orphaned saga reservation routing, Cashu Request payment orchestration, external top-up quote creation, mint quote settlement policy, receive-fee fallback estimation, Mint Detail display mapping and connection-state tests, Mint Detail Compose rendering coverage for NUT-06 metadata, Receive Lightning polling cadence tests, and Receive Lightning quote-flow tests for quote intent persistence, force-new on-chain creation, reusable BOLT12 reuse, and settlement attachment. Direct AndroidJUnitRunner instrumentation on the connected emulator now passes 49/49 tests after fixing fake-shell route assertions, split visual probes, compact-height scrolling, settings toggle state, and mint swipe idempotence. Standard Gradle connected result collection, Gradle-managed-device instrumentation, and live native wallet-operation instrumentation pass.
...
- [x] Add receive Lightning full screen/integration tests for method picker, BOLT11 invoice display/expiry, BOLT12 reusable offer editing, on-chain observer/link, success/failure status, and back behavior. `FakeWalletParityComposeTest` covers these Receive Lightning states through the app-level fake Compose shell; direct connected-emulator execution passes.
...
Milestone update: local Nutshell setup now self-selects a compatible Python 3.10-3.12 runtime, recreates incompatible virtualenvs, runs hermetically from `CI/.nutshell-workdir` instead of user-level Cashu config, binds to local-only host settings, and uses deterministic FakeWallet quote settlement. CDK setup is now executable/idempotent, writes a bootable fakewallet config with local-only seed material, exposes sat and usd fakewallet units, and has been smoke-started successfully against `/v1/info`. Android has a host-safe live local-mint Gradle task, `:app:androidLocalMintIntegrationTest`, which passed locally against Nutshell and CDK for `/v1/info`, `/v1/keys`, BOLT11 quotes, CDK BOLT12 offers, CDK on-chain quotes, CDK usd quotes, Cashu Request decoding, and Cashu token prefix parsing. Android also now has opt-in native on-device local-mint instrumentation, `NativeWalletLocalMintInstrumentedTest`, which passed through Gradle connected instrumentation against forwarded Nutshell/CDK mints for native wallet open, fetch info, BOLT11 mint, BOLT11 melt with a fresh generated invoice, restore, plain ecash receive, P2PK locked-token parse/receive, CDK BOLT11 sat mint, CDK usd mint, CDK BOLT12 quote parsing, CDK on-chain quote creation, and Cashu payment request decode. SwiftPM iOS integration tests also pass 12/12 against the hermetic Nutshell FakeWallet mint.
...
- [x] Run the full Android wallet-operation matrix against local Nutshell/CDK test mints for mint, melt, restore, token parser, payment request parser, multi-unit, BOLT11, BOLT12, and on-chain where available. `NativeWalletLocalMintInstrumentedTest` passes under Gradle connected instrumentation with `adb reverse` and a fresh generated BOLT11 melt invoice; the test intentionally treats CDK proof-state checks as a no-crash/pending-status guard because this CDK runtime does not report sent-token proof state as a reliable spent transition after successful receive.
...
Milestone update: local Gradle release gates passed for the current branch with `:app:testDebugUnitTest`, `:app:lintDebug`, and `:app:assembleRelease`. Local Android endpoint integration also passed with `:app:compileDebugUnitTestKotlin :app:androidLocalMintIntegrationTest` against running Nutshell/CDK test mints. Direct connected-emulator AndroidJUnitRunner instrumentation passed 49/49 tests after fixing Compose harness, compact scrolling, settings toggle, and mint swipe regressions. Standard Gradle connected instrumentation result collection is restored on a normal attached AVD selected with `ANDROID_SERIAL=<attached-emulator-serial>`; the full app instrumentation suite finished 51 tests with 1 opt-in native matrix test skipped and 0 failures, and the focused native wallet local-mint matrix collects results successfully when explicitly enabled. Gradle-managed-device instrumentation is also restored: `:app:pixel2Api35DebugAndroidTest` passed after setting the managed emulator GPU mode to `swiftshader_indirect`, finishing 51 tests with 1 opt-in native matrix test skipped and 0 failures. SwiftPM iOS integration tests passed 12/12 after hardening the local Nutshell FakeWallet launcher, and Xcode project/unit/UI-test discovery now completes after resolving Swift packages. Real-mint payment validation and physical-device manual parity walkthroughs remain open external acceptance gates.
...
- [x] Restore standard Gradle connected instrumentation result collection. `ANDROID_SERIAL=<attached-emulator-serial> JAVA_HOME="$JAVA_HOME" ./gradlew --no-daemon :app:connectedDebugAndroidTest` passed on a normal attached API 35 AVD, finishing 51 tests with 1 opt-in native matrix test skipped and 0 failures; the same Gradle connected path passes the focused native wallet local-mint instrumentation class when explicitly enabled.
- [x] Restore Gradle-managed-device instrumentation result collection. `android.testoptions.manageddevices.emulator.gpu=swiftshader_indirect` avoids the emulator 36.6.11 `auto-no-window` rejection; after `:app:cleanManagedDevices`, `JAVA_HOME="$JAVA_HOME" ./gradlew --no-daemon :app:pixel2Api35DebugAndroidTest -Pandroid.experimental.testOptions.managedDevices.setupTimeoutMinutes=15` passed, finishing 51 tests with 1 opt-in native matrix test skipped and 0 failures.
...
- [x] Restore full Xcode project/unit/UI-test discovery. `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -list -project CashuWallet.xcodeproj -clonedSourcePackagesDirPath <temporary-package-cache>` resolves the Swift package graph and lists the `CashuWallet`, `CashuWalletTests`, and `CashuWalletUITests` targets plus the `CashuWallet` scheme.
- [ ] Perform manual parity walkthrough on physical Android device: onboarding, restore, backup/security, home, send, receive, scanner, NFC, mints, history, settings. External gate: requires a physical Android device with camera/NFC and release-candidate build installed.
- [ ] Perform manual parity walkthrough on iOS after any shared model/protocol changes. External gate: requires an iOS simulator/device pass from the product owner or release tester after Android branch review.
- [ ] Validate with at least one real mint supporting current CDK features, one BOLT11 path, one BOLT12 path, one on-chain path, one P2PK locked token, and one Cashu Request paid over Nostr. External gate: requires real mint access, relay/payment infrastructure, and release-acceptance funds.
- [x] Verify no PII/secrets/tokens/seeds/private keys appear in logs, screenshots, crash reports, commits, PRs, or release notes. Commit subjects were scanned for local paths, device identifiers, token prefixes, seed/private-key vocabulary, and nsec prefixes with no findings. Tracked-file scans found no user-local paths; remaining token/nsec matches are intentional examples, labels, sanitizer tests, and parser fixtures. Touched-file secret scans contain only expected test/generated integration values such as deterministic local-only invoice signing, generated mnemonic variables, P2PK test keys, and redaction coverage.
