...
class WalletManager(
    ...
    suspend fun existingAmountlessOffer(): MintQuoteInfo? =
        withLoadingResult {
            ...
            selectReusableAmountlessOffer(gateway.listUnissuedMintQuotes(), active.url)
        }

    suspend fun existingOnchainMintQuote(): MintQuoteInfo? =
        withLoadingResult {
            val active = mutableState.value.activeMint ?: return@withLoadingResult null
            selectReusableOnchainMintQuote(gateway.listUnissuedMintQuotes(), active.url)
        }
    ...
    private suspend fun performBestEffortWalletStartupMaintenance() {
        runBestEffortWalletStartupMaintenance(
            trackedMints = walletStore.loadMints(),
            ensureWallet = { mintUrl, unit -> gateway.ensureWallet(mintUrl, unit) },
            refreshBalance = { refreshBalance() },
            loadTransactions = { loadTransactions() },
            performForegroundMaintenance = { performForegroundMaintenance() },
            onError = { message, error -> AppLogger.wallet.error(message, error) },
        )
    }
